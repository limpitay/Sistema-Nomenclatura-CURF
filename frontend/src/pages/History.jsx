...
const ESTADOS = ['todos', 'activo', 'reasignado', 'baja'];
...
export default function History() {
  ...
  const fetchHostnames = async () => {
    ...
    try {
      ...
      const res = await client.get('/hostnames', { params });
  ...
  useEffect(() => { fetchHostnames(); }, [estado]);
  ...
  const openDetail = async (hn) => {
    ...
    setLoadingEv(true);
    try {
      const res = await client.get(`/hostnames/${hn.hostname}/events`);
      setEvents(res.data);
    } catch {
      setEvents([]);
    } finally {
  ...
  const changeEstado = async (id, nuevoEstado) => {
    ...
    try {
      await client.patch(`/hostnames/${id}`, { estado: nuevoEstado, notas: motivo });
      setSelected(null);
      fetchHostnames();
    } catch (err) {
  ...
  const estadoBadge = (e) => {
    const map = {
      activo:     { bg:'#e6f5ec', color:'#1a7a3c', label:'Activo'     },
      reasignado: { bg:'#fdf3dc', color:'#8a5a00', label:'Reasignado' },
      baja:       { bg:'#f0efe9', color:'#999',     label:'Baja'       },
    };
    const st = map[e] || map.baja;
    return (
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <div style={s.headerRight}>
          ...
          <button style={s.btnGhost} onClick={() => navigate('/')}>Builder</button>
          ...
          <div style={s.filterRow}>
            <form onSubmit={handleSearch} style={s.searchForm}>
              <input
                ...
                placeholder="Buscar hostname, usuario, sector…"
              />
              <button type="submit" style={s.btnPrimary}>Buscar</button>
              {search && (
                <button type="button" style={s.btnGhost}
                  onClick={() => { setSearch(''); setTimeout(fetchHostnames, 0); }}>
                  Limpiar
                </button>
              )}
            </form>
          ...
          {loading ? (
            ...
            <div style={{overflowX:'auto'}}>
              <table style={s.table}>
                <thead>
                  <tr>
                    {['Hostname','Tipo','Ubicación','Usuario Windows','Técnico','Fecha','Estado',''].map(h => (
                  ...
                  {hostnames.map(hn => (
                    <tr key={hn.id} style={s.tr}>
                      <td style={s.td}>
                        ...
                        <div style={s.hnDisplay}>{hn.hostname_display}</div>
                      </td>
                      <td style={s.td}>{hn.tipo}</td>
                      <td style={s.td}>
                        <span style={s.mono}>{hn.edificio} {hn.piso}</span>
                        <div style={{fontSize:11,color:'#999'}}>{hn.sector}</div>
                      </td>
                      <td style={s.td}>{hn.usuario_windows || '—'}</td>
                      <td style={{...s.td, fontSize:11, color:'#666'}}>{hn.tecnico_nombre || '—'}</td>
                      <td style={{...s.td, fontSize:11, fontFamily:'monospace'}}>{fmtDate(hn.created_at)}</td>
                      <td style={s.td}>{estadoBadge(hn.estado)}</td>
                      <td style={s.td}>
        ...
        <div style={s.overlay} onClick={() => setSelected(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.modalHeader}>
              <div style={s.modalHn}>{selected.hostname}</div>
            ...
            <div style={s.infoGrid}>
              {[
                ...
                ['Técnico',         selected.tecnico_nombre || '—'],
                ['Serie',           selected.numero_serie || '—'],
              ...
              {selected.notas && (
                <div style={{...s.infoItem, gridColumn:'1/-1'}}>
                  <div style={s.infoLabel}>Notas</div>
                  <div style={s.infoVal}>{selected.notas}</div>
                </div>
              )}
            </div>
            ...
            {selected.estado === 'activo' && (
              <div style={{display:'flex', gap:8, margin:'12px 0'}}>
                <button style={s.btnGhost}
                  onClick={() => changeEstado(selected.id, 'reasignado')}>
                  Marcar reasignado
                </button>
                <button style={{...s.btnGhost, color:'#b83030', borderColor:'#f0b0b0'}}
                  onClick={() => changeEstado(selected.id, 'baja')}>
                  Dar de baja
                </button>
              </div>
            )}
            ...
            <div style={s.evTitle}>Historial</div>
            {loadingEv ? (
